...
for i in range(jum_baris):
    for j in range(jum_kolom):
        citra_multiplication[i, j, 0] = b1[i, j] / divider
        citra_multiplication[i, j, 1] = g1[i, j] / divider
        citra_multiplication[i, j, 2] = r1[i, j] / divider
...
cv2.imshow("division", citra_multiplication)
